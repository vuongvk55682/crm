...
Class Mongo_db{
	...
	*/
	...
	public function where1($wheres, $value = null)
	{
		if (is_array($wheres))
		{
			foreach ($wheres as $wh => $val)
	...
	public function where2($wheres, $value = null)
	{
		if (is_array($wheres))
		{
			foreach ($wheres as $wh => $val)
	...
	public function where3($wheres, $value = '')
	{
		if (is_array($wheres))
		{
			foreach ($wheres as $wh => $val)
			{
				...
				{
					$this->wheres[$wheres] = new MongoRegex("/".$value.".*$/i");
				}
			}
		}
		else
	...
	*/
	...
	public function where_in1($field = "", $in = array())
	{	
		if($in != array('0'))
		{
			if (empty($field))
	...
	*/
	...
	public function where_multiple($dieukien, $value = null)
	{
		if (!isset($dieukien))
		{
		...
		foreach ($dieukien as $key => $value) {
			...
			$this->_w($key);
				if($value[1]=='bang'){
				$this->wheres[$key] = $value[0];		
				}
				else if($value[1]=='lonhon'){
				$this->wheres[$key]['$gte'] = $value[0];		
				}
				else if($value[1]=='nhohon'){
				$this->wheres[$key]['$lte'] = $value[0];			
				}				
			} 
		}
		return ($this);
	...
	*/
	...
	public function where_between1($field = "", $x, $y)
	{
		if(isset($x)&&isset($y)){
			$this->_w($field);
			$this->wheres[$field]['$gte'] = $x;
	...
	*/
	...
	public function get1($collection = "",$limit = 50, $offset = 0 )
	{			
		if (empty($collection))
		{
			show_error("In order to retrieve documents from MongoDB, a collection name must be passed", 500);
	...
	*/
	...
	public function count1($collection = "", $limit = NULL) 
	{
		if (empty($collection))
		{
			show_error("In order to retrieve a count of documents from MongoDB, a collection name must be passed", 500);
		}
		$count = $this->db->{$collection}->find($this->wheres)->limit((int)$limit)->skip((int) $this->offset)->count();
		$this->_clear();
		return ($count);
	}
	...
	*/
	...
	// 	}
	// }
	public function update($collection = "", $updates = NULL, $wheres = NULL, $options = array())
	{
		if (empty($collection))
		{
			show_error("No Mongo collection selected for update", 500);
		...
		{
			$options = array_merge(array('w' => $this->write_concerns, 'j'=>$this->journal, 'multiple' => FALSE), $options);
			$this->db->{$collection}->update($wheres, $updates, $options);
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	public function update_set($collection = "",$data,$id)
    {
        if (empty($collection))
		{
			show_error("No Mongo collection selected for update", 500);
		...
		try
		{
            ...
            $this->db->{$collection}->update($where,$update);
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	public function update_set1($collection = "",$data,$where = NULL)
    {
        if (empty($collection))
		{
			show_error("No Mongo collection selected for update", 500);
	...
	*/
	...
	// 	}
	// }
	public function delete($collection = "",$wheres = NULL, $options = NULL)
	{
		if (empty($collection))
		{
			show_error("No Mongo collection selected to delete from", 500);
		...
		try
		{
			$options = array('w' => $this->write_concerns, 'j'=>$this->journal, 'justOne' => TRUE);
			$this->db->{$collection}->remove($wheres, $options);
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	*/
	...
	public function delete_all1($collection = "",$wheres = NULL)
	{
		if (empty($collection))
		{
			show_error("No Mongo collection selected to delete from", 500);
		...
		try
		{
			$options = array('w' => $this->write_concerns, 'j'=>$this->journal, 'justOne' => FALSE);
			$this->db->{$collection}->remove($wheres, $options);
			$this->_clear();
			return (TRUE);
		}
