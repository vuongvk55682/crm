...
class Slider extends MY_Controller {
	...
	public function index($page = 1)
	{
		...
		$config['total_rows'] = $this->query_sql->total_where('ci_image',array('type' => 'slider'));
		...
		if($config['total_rows']>0){
			$data['slider'] = $this->query_sql->view_where('id, name, created, publish, sort,image, url','ci_image',array('type' => 'slider'),($page * $config['per_page']),$config['per_page']);
		}
		...
		if($this->input->post()){
			...
			if($this->form_validation->run()){
				if($_FILES["image"]["name"]){
					...
					$config['source_image'] = 'upload/slider/'.$image_data['file_name'];
					$config['new_image'] = 'upload/slider/thumb/'.$image_data['file_name'];
				...
				if($flag>0){
					...
					redirect('admin/slider/index',$data);
				}else{
					$this->session->set_flashdata('message_flashdata', array(
						'type'		=> 'error',
						'message'	=> 'Thất bại!',
	...
	public function edit($id=0)
	{
		...
		if($this->input->post()){
			...
			if($this->form_validation->run()){
				if($_FILES["image"]["name"]){
					...
					$file = "upload/slider/".$data['image'];
					$file_thumb = "upload/slider/thumb/".$data['image_thumb'];
					unlink($file);
					unlink($file_thumb);
		...
		$data['slider'] = $this->query_sql->select_row('ci_image', 'id, name,url,image,width,height,type,auto, created, publish', array('id' => $id));
	...
	public function show()
	{
		...
		$this->load->view('backend/slider/show', $data_publish);
	}
	public function showall()
	{
		...
		$data_publish['publish'] = $data_sql['publish'];
		$this->load->view('backend/slider/showall', $data_publish);
	}
    ...
    public function search()
    {
    	...
    	$data['slider'] = $this->query_sql->select_like_where('ci_image','id, name, created, publish, sort,image, url',array('type' => 'slider'),$data_like,'');
		$this->load->view('backend/slider/search', isset($data)?$data:NULL);
    }
}
