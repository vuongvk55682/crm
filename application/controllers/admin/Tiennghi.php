...
class Tiennghi extends MY_Controller {
	...
	public function index($page = 1)
	{
		...
		$data['title'] = 'Quản lý tiện nghi';
		
		$config = $this->query_sql->_pagination();
		$config['base_url'] = base_url().'admin/tiennghi/index/';
		$config['total_rows'] = $this->query_sql->total_where('ci_image',array('type' => 'tiennghi'));
		...
		if($config['total_rows']>0){
			$data['tiennghi'] = $this->query_sql->view_where('id, name, created, publish, sort,image, url','ci_image',array('type' => 'tiennghi'),($page * $config['per_page']),$config['per_page']);
		}
		
		$data['template'] = 'backend/tiennghi/index';
	...
	public function add()
	{
		...
		$data['title'] = 'Thêm tiện nghi';
		$data['template'] = 'backend/tiennghi/add';
		if($this->input->post()){
			$this->form_validation->set_rules('name','Tên','trim|required');
			if($this->form_validation->run()){
				if($_FILES["image"]["name"]){
					$album_dir = 'upload/tiennghi/';
					...
					$config['source_image'] = 'upload/tiennghi/'.$image_data['file_name'];
					$config['new_image'] = 'upload/tiennghi/thumb/'.$image_data['file_name'];
				...
				if($flag>0){
					...
					redirect('admin/tiennghi/index',$data);
				}else{
					$this->session->set_flashdata('message_flashdata', array(
						'type'		=> 'error',
						'message'	=> 'Thất bại!',
	...
	public function edit($id=0)
	{
		...
		if($this->input->post()){
			...
			if($this->form_validation->run()){
				if($_FILES["image"]["name"]){
					...
					$file = "upload/tiennghi/".$data['image'];
					$file_thumb = "upload/tiennghi/thumb/".$data['image_thumb'];
					unlink($file);
					unlink($file_thumb);
		...
		$data['tiennghi'] = $this->query_sql->select_row('ci_image', 'id, name,url,image,width,height,type,auto, created, publish', array('id' => $id));
		$data['title'] = 'Cập nhật tiennghi';
		$data['template'] = 'backend/tiennghi/edit';
	...
	public function show()
	{
		...
		$this->load->view('backend/tiennghi/show', $data_publish);
	}
	public function showall()
	{
		...
		$data_publish['publish'] = $data_sql['publish'];
		$this->load->view('backend/tiennghi/showall', $data_publish);
	}
    ...
    public function search()
    {
    	...
    	$data['tiennghi'] = $this->query_sql->select_like_where('ci_image','id, name, created, publish, sort,image, url',array('type' => 'tiennghi'),$data_like,'');
		$this->load->view('backend/tiennghi/search', isset($data)?$data:NULL);
    }
}
