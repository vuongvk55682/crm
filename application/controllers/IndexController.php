...
class IndexController extends MY_Controller {
	...
	public function index()
	{
		...
		// $this->mongo_db->insert('khachhang',array('name' => 'Nguyễn Văn A', 'age' => 20, 'gioitinh' => 'Nữ' ));

		
		// var_dump($a);

		// $this->mongo_db->delete_all('khachhang',array('gioitinh' => 'Nữ' ));
		// $this->mongo_db->select('gioitinh1')
		// $this->mongo_db->delete_all('khachhang', array());
		// $this->mongo_db->where()
		// $a = $this->mongo_db->select(array('age'))->where(array('gioitinh' => 'Nam'))->get('khachhang');
		// var_dump($a);
		// $c = $this->mongo_db->get('khachhang');
		// // var_dump($c);

		// $a=$this->mongo_db->get_where('khachhang',array('gioitinh' => 'Nam'));
		// // var_dump($a);
		...
		// 	$this->mongo_db->unset_field($val['age'])->update('khachhang');
		// }
		
		// $a = $this->mongo_db->find_one('khachhang');
		// var_dump($a);
	...
	public function sentmail(){
		if($this->input->post()){
			...
			$data_mail = $this->query_sql->select_row('dev_mail', 'id', array('email' => $mail));
			if($data_mail==NULL){
				$data_insert = array(
					'email' 			=> 	$mail,
	...
	public function detailroute($alias = '', $page = 1){
	...
	public function reorder(){
		if($this->input->post()){
			...
			$data_insert = array(
				...
				'dichvusuachuaid' 			=> 	$this->input->post('dichvusuachuaid'),
				'position' 		=> 	$position,
				'address' 		=> 	$address,
				'content' 		=> 	$this->input->post('content'),
				'created'		=>	gmdate('Y-m-d H:i:s', time()+7*3600)
			);
			$flag = $this->query_sql->add('dev_reorder', $data_insert);
			redirect(base_url());
		}
	}
